...
impl Row {
    pub fn new(cols: u16) -> Self {
        Self {
            cells: vec![super::Cell::new(); usize::from(cols)],
            wrapped: false,
        }
    }
    ...
    fn cells(&self) -> impl Iterator<Item = &super::Cell> {
        self.cells.iter()
    }

    pub fn get(&self, col: u16) -> Option<&super::Cell> {
        self.cells.get(usize::from(col))
    }

    pub fn get_mut(&mut self, col: u16) -> Option<&mut super::Cell> {
        self.cells.get_mut(usize::from(col))
    }

    pub fn insert(&mut self, i: u16, cell: super::Cell) {
    ...
    pub fn resize(&mut self, len: u16, cell: super::Cell) {
    ...
    pub fn write_contents_formatted(
        ...
        row: u16,
        wrapping: bool,
        prev_pos: Option<super::grid::Pos>,
        prev_attrs: Option<super::attrs::Attrs>,
    ) -> (super::grid::Pos, super::attrs::Attrs) {
        let mut prev_was_wide = false;
        let default_cell = super::Cell::new();
        ...
        if wrapping && first_cell == &default_cell {
            ...
            prev_pos = super::grid::Pos { row, col: 0 };
        }

        let mut erase: Option<(u16, &super::attrs::Attrs)> = None;
        for (col, cell) in self
            .cells()
            .enumerate()
            .skip(usize::from(start))
            .take(usize::from(width))
        {
            ...
            // we limit the number of cols to a u16 (see Size)
            let col: u16 = col.try_into().unwrap();
            let pos = super::grid::Pos { row, col };

            if let Some((prev_col, attrs)) = erase {
                if cell.has_contents() || cell.attrs() != attrs {
                    let new_pos = super::grid::Pos { row, col: prev_col };
                    if wrapping && prev_pos.row + 1 == new_pos.row && prev_pos.col >= self.cols() {
                        ...
                        super::term::MoveFromTo::new(prev_pos, new_pos).write_buf(contents);
                    }
                    prev_pos = new_pos;
                    if &prev_attrs != attrs {
                    ...
                    super::term::EraseChar::new(pos.col - prev_col).write_buf(contents);
                    erase = None;
                }
            }
        ...
        if let Some((prev_col, attrs)) = erase {
            ...
            super::term::ClearRowForward.write_buf(contents);
        }
    ...
    pub fn write_contents_diff(
        ...
        if (!self.wrapped && prev.wrapped) || (!prev.wrapped && self.wrapped) {
            ...
            super::term::MoveFromTo::new(prev_pos, end_pos).write_buf(contents);
            prev_pos = end_pos;
            if !self.wrapped {
                super::term::EraseChar::new(1).write_buf(contents);
            }
