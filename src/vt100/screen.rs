...
impl Screen {
    pub(crate) fn new(size: super::grid::Size, scrollback_len: usize) -> Self {
    ...
    fn write_contents_formatted(&self, contents: &mut Vec<u8>) {
        super::term::HideCursor::new(self.hide_cursor()).write_buf(contents);
        ...
        self.grid()
            ...
            .map(move |(i, (row, prev_row))| {
                ...
                row.write_contents_diff(
                    ...
                    false,
                    false,
                    super::grid::Pos { row: i, col: start },
                    super::attrs::Attrs::default(),
                );
                contents
            })
    }
    ...
    fn write_input_mode_formatted(&self, contents: &mut Vec<u8>) {
        super::term::ApplicationKeypad::new(self.mode(MODE_APPLICATION_KEYPAD)).write_buf(contents);
        super::term::ApplicationCursor::new(self.mode(MODE_APPLICATION_CURSOR)).write_buf(contents);
        super::term::BracketedPaste::new(self.mode(MODE_BRACKETED_PASTE)).write_buf(contents);
        super::term::MouseProtocolMode::new(self.mouse_protocol_mode, MouseProtocolMode::None)
            .write_buf(contents);
        super::term::MouseProtocolEncoding::new(
    ...
    fn write_input_mode_diff(&self, contents: &mut Vec<u8>, prev: &Self) {
        if self.mode(MODE_APPLICATION_KEYPAD) != prev.mode(MODE_APPLICATION_KEYPAD) {
            super::term::ApplicationKeypad::new(self.mode(MODE_APPLICATION_KEYPAD))
                .write_buf(contents);
        }
        if self.mode(MODE_APPLICATION_CURSOR) != prev.mode(MODE_APPLICATION_CURSOR) {
            super::term::ApplicationCursor::new(self.mode(MODE_APPLICATION_CURSOR))
                .write_buf(contents);
        }
        if self.mode(MODE_BRACKETED_PASTE) != prev.mode(MODE_BRACKETED_PASTE) {
            super::term::BracketedPaste::new(self.mode(MODE_BRACKETED_PASTE)).write_buf(contents);
        }
        super::term::MouseProtocolMode::new(self.mouse_protocol_mode, prev.mouse_protocol_mode)
            .write_buf(contents);
        super::term::MouseProtocolEncoding::new(
            self.mouse_protocol_encoding,
    ...
    fn write_attributes_formatted(&self, contents: &mut Vec<u8>) {
        super::term::ClearAttrs.write_buf(contents);
        self.attrs
            .write_escape_code_diff(contents, &super::attrs::Attrs::default());
    }
    ...
    /// Returns the [`Cell`](super::Cell) object at the given location in the
    /// terminal, if it exists.
    #[must_use]
    pub fn cell(&self, row: u16, col: u16) -> Option<&super::Cell> {
    ...
    pub fn row_wrapped(&self, row: u16) -> bool {
        self.grid()
            .visible_row(row)
            .is_some_and(super::row::Row::wrapped)
    }
    ...
    #[must_use]
    pub fn fgcolor(&self) -> super::Color {
        self.attrs.fgcolor
    }

    /// Returns the currently active background color.
    #[must_use]
    pub fn bgcolor(&self) -> super::Color {
        self.attrs.bgcolor
    }
    ...
    pub(crate) fn grid(&self) -> &super::grid::Grid {
        ...
        loop {
            match next_param!() {
                ...
                [n] if (30..=37).contains(n) => {
                    self.attrs.fgcolor = super::Color::Idx(to_u8!(*n) - 30);
                }
                [38, 2, r, g, b] => {
                    self.attrs.fgcolor = super::Color::Rgb(to_u8!(*r), to_u8!(*g), to_u8!(*b));
                }
                [38, 5, i] => {
                    self.attrs.fgcolor = super::Color::Idx(to_u8!(*i));
                }
                [38] => match next_param!() {
                    [2] => {
                        let r = next_param_u8!();
                        let g = next_param_u8!();
                        let b = next_param_u8!();
                        self.attrs.fgcolor = super::Color::Rgb(r, g, b);
                    }
                    [5] => {
                        self.attrs.fgcolor = super::Color::Idx(next_param_u8!());
                    }
                    _ => {
                        unhandled(self);
                        return;
                ...
                [39] => {
                    self.attrs.fgcolor = super::Color::Default;
                }
                [n] if (40..=47).contains(n) => {
                    self.attrs.bgcolor = super::Color::Idx(to_u8!(*n) - 40);
                }
                [48, 2, r, g, b] => {
                    self.attrs.bgcolor = super::Color::Rgb(to_u8!(*r), to_u8!(*g), to_u8!(*b));
                }
                [48, 5, i] => {
                    self.attrs.bgcolor = super::Color::Idx(to_u8!(*i));
                }
                [48] => match next_param!() {
                    [2] => {
                        let r = next_param_u8!();
                        let g = next_param_u8!();
                        let b = next_param_u8!();
                        self.attrs.bgcolor = super::Color::Rgb(r, g, b);
                    }
                    [5] => {
                        self.attrs.bgcolor = super::Color::Idx(next_param_u8!());
                    }
                    _ => {
                        unhandled(self);
                        return;
                ...
                [49] => {
                    self.attrs.bgcolor = super::Color::Default;
                }
                [n] if (90..=97).contains(n) => {
                    self.attrs.fgcolor = super::Color::Idx(to_u8!(*n) - 82);
                }
                [n] if (100..=107).contains(n) => {
                    self.attrs.bgcolor = super::Color::Idx(to_u8!(*n) - 92);
                }
                _ => unhandled(self),
            }
        }
