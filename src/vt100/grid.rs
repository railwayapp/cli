...
impl Grid {
    ...
    pub fn set_size(&mut self, size: Size) {
        ...
        self.size = size;
        for row in &mut self.rows {
            row.resize(size.cols, super::Cell::new());
        }
    ...
    pub fn visible_cell(&self, pos: Pos) -> Option<&super::Cell> {
        self.visible_row(pos.row).and_then(|r| r.get(pos.col))
    }

    pub fn drawing_cell(&self, pos: Pos) -> Option<&super::Cell> {
        self.drawing_row(pos.row).and_then(|r| r.get(pos.col))
    }

    pub fn drawing_cell_mut(&mut self, pos: Pos) -> Option<&mut super::Cell> {
    ...
    pub fn write_contents_formatted(&self, contents: &mut Vec<u8>) -> super::attrs::Attrs {
        super::term::ClearAttrs.write_buf(contents);
        super::term::ClearScreen.write_buf(contents);

        let mut prev_attrs = super::attrs::Attrs::default();
    ...
    pub fn write_contents_diff(
        ...
        contents: &mut Vec<u8>,
        prev: &Self,
        mut prev_attrs: super::attrs::Attrs,
    ) -> super::attrs::Attrs {
    ...
    pub fn write_cursor_position_formatted(
        ...
        contents: &mut Vec<u8>,
        prev_pos: Option<Pos>,
        prev_attrs: Option<super::attrs::Attrs>,
    ) {
        ...
        if prev_pos != Some(self.pos) && self.pos.col >= self.size.cols {
            ...
            if cell.has_contents() {
                if let Some(prev_pos) = prev_pos {
                    super::term::MoveFromTo::new(prev_pos, pos).write_buf(contents);
                } else {
                    super::term::MoveTo::new(pos).write_buf(contents);
                }
                ...
                if !found {
                    ...
                    super::term::SaveCursor.write_buf(contents);
                    super::term::Backspace.write_buf(contents);
                    super::term::EraseChar::new(1).write_buf(contents);
                    super::term::RestoreCursor.write_buf(contents);
                    prev_attrs.write_escape_code_diff(contents, end_cell.attrs());
                }
            }
        } else if let Some(prev_pos) = prev_pos {
            super::term::MoveFromTo::new(prev_pos, self.pos).write_buf(contents);
        } else {
            super::term::MoveTo::new(self.pos).write_buf(contents);
        }
    }
    ...
    pub fn insert_cells(&mut self, count: u16) {
        ...
        for _ in 0..count {
            ...
            row.insert(pos.col, super::Cell::new());
            if wide {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // Inline TUIs such as Codex reserve rows below the scrolling
            // output for a composer. Lines leaving the top of the primary
            // screen still belong in terminal history. Regions below a fixed
            // header and alternate screens (zero history capacity) do not.
