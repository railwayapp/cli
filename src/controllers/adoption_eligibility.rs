...
mod tests {
    ...
    /// The mysql-pitr root slot's real declaration: same entrypoint rule, but
    /// NO floating-major requirement -- its image matrix publishes exact
    /// minors, so a minor tag is the normal case rather than a blocker.
    fn mysql_pitr_rules() -> AdoptionRules {
        rules_from_template(&json!({
            "services": {
                "root": {
                    "clusterRole": "root",
    ...
    #[test]
    fn mysql_minor_pins_are_accepted_because_its_matrix_publishes_them() {
        // The exact divergence a hardcoded "minor pins are bad" rule would
        // have caused: mysql-ha/mysql only ever publishes major.minor.
        let rules = mysql_pitr_rules();
        assert!(
            rules
                .blockers(&target(
