...
/// True when this service IS the engine's pooler, by image -- the same
/// discriminator `compute_pooling_state` attaches with. The pooler hangs off
/// the root exactly like an HA member does (parent = root, role "edge"), so
/// anything walking cluster membership to DELETE services must ask this: the
/// pooler belongs to the pooling feature, not to the HA conversion, and the
/// engine's `pool remove` is its only legitimate remover.
pub fn is_pooler_service(service: &ServiceInstance, pooling: &PoolingSpec) -> bool {
    service
        .source
        .as_ref()
...
pub fn compute_pooling_state(
    ...
    let edge = config.services.iter().find(|(_, service)| {
        !service.is_deleted.unwrap_or(false)
            ...
            && is_pooler_service(service, pooling)
    });
