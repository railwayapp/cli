...
async fn dry_run(args: &Args, apps: &[App], home: &Path, ssh_config_path: &Path) -> Result<()> {
    ...
    if apps.contains(&App::Codex) {
        if apps == [App::Codex] {
            println!(
                "Would start or reuse the authenticated Codex App Server on the code endpoint (port 4096; legacy connections keep 8080) and verify its public WebSocket endpoint."
            );
        }
    ...
    if apps.iter().any(|app| app.is_opencode()) {
        ...
        println!(
            "Would start password-protected OpenCode in the background on the code endpoint (port 4096; legacy connections keep 8080) from {}.",
            args.dir
        );
        println!(
