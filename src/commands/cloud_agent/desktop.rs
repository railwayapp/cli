...
async fn dry_run(args: &Args, apps: &[App], home: &Path, ssh_config_path: &Path) -> Result<()> {
    ...
    if apps.contains(&App::OpenCode) {
        ...
        println!(
            ...
            opencode_config::config_dir()?.display()
        );
        println!(
