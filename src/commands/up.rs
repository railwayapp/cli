...
pub struct Args {
    ...
    #[clap(long)]
    /// Use path as the prefix for the archive (project directory is the default)
    use_path_as_archive_prefix: bool,
...
fn get_deploy_paths(args: &Args, linked_project: &LinkedProject) -> Result<DeployPaths> {
    if args.use_path_as_archive_prefix {
        if args.path.is_none() {
            bail!("--use-path-as-prefix requires a path to be specified");
        }
