...
pub async fn new_environment(args: Args) -> Result<()> {
    ...
    if let Some(ref source_env_id) = duplicate_id {
        ...
        let source_config =
            fetch_environment_config(&client, &configs, source_env_id, true).await?;

        // Get any --service-config overrides
        let override_config =
            edit_services_select(&args, &client, &configs, &project, source_env_id.clone()).await?;

        // Merge source config with overrides (overrides take precedence)
        let merged_config = merge_configs(source_config.config, override_config);
