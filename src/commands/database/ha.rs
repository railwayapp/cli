...
async fn revert(
    ...
    // parentServiceId on stragglers, so they can't be re-derived from the
    // fresh config.
    let mut leftovers: Vec<(String, String)> = pre_revert_members
        ...
        .collect();
    // The public patch path DROPS parentServiceId (confirmed live: staging it
    // round-trips clusterRole but not the parent link), so a node added by live
    // scaling is invisible to the membership snapshot too. A role-stamped
    // service with NO parent is not a legitimate end state of any flow -- treat
    // those as cluster debris and sweep them as well.
    let names = service_name_map(&ctx);
